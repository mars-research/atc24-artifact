#include "maglev.h"
#include <stdio.h>
static const uint64_t ETH_HEADER_LEN = 14;
static const uint64_t UDP_HEADER_LEN = 8;

// https://en.wikipedia.org/wiki/IPv4
static const uint64_t IPV4_PROTO_OFFSET = 9;
static const uint64_t IPV4_LENGTH_OFFSET = 2;
static const uint64_t IPV4_CHECKSUM_OFFSET = 10;
static const uint64_t IPV4_SRCDST_OFFSET = 12;
static const uint64_t IPV4_SRCDST_LEN = 8;
static const uint64_t UDP_LENGTH_OFFSET = 4;
static const uint64_t UDP_CHECKSUM_OFFSET = 6;
...
static void  swap_mac(struct rte_ether_hdr *eth_hdr) {
  uint8_t 	tmp;
...
static __inline__ uint64_t  flowhash(void *frame, size_t len) {
...
// Eisenbud 3.4
static void  populate_lut(LookUpTable lut) {
...
static void maglev_hashmap_insert(uint64_t key, uint64_t value)
{
...
static struct maglev_kv_pair* maglev_hashmap_get( uint64_t key)
{
	//uint64_t hash = hash_fn(key);
	uint64_t hash = fnv_1((char*)&key, sizeof(key));
	for (uint64_t i = 0; i < CAPACITY; ++i) {
